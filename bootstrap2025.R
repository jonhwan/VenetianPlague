...
entropyprocedure_2 <- function(original_data,indices,timestep,stat=c("Shannon","Cases","Negent","UniformKL","Shannon_and_Cases","Negent_and_Cases")){ #might need to transpose for bootstrap to work
  ...
  if(stat == "Cases"){
    ...
    if(stat == "Shannon_and_Cases"){
      ...
      x <- seq(0,max(cv),length.out=length(kern)) #originally had length(kern)-1
      y <- dnorm(x,mean=mean(kern),sd=sd(kern))
      y <- y/sum(y)
      #v <- KL_new(kern,y) #new approach
...
manual_boot <- function(data,N,ts,st){ #timestep, stat
  vec <- rep(0,N)
  for(i in 1:N){
    vec[i] <- entropyprocedure_2(data,sample(ncol(data),replace=TRUE),ts,st)
  }
  return(vec)
}
...
manual_boot_two_outputs <- function(data,N,ts,st){
  ...
  v2 <- rep(0,N)
  for(i in 1:N){
    x <- entropyprocedure_2(data,sample(ncol(data),replace=TRUE),ts,st)
    v1[i] <- x[1]
    v2[i] <- x[2]
  }
  return(cbind(v1,v2))
...
bias <- rep(0,length(filled$Date)) #is this allowed? I mean it makes sense if you assume it's normally distributed or whatever, but what if the underlying distribution isn't normal?
s_original <- rep(0,length(filled$Date)) #used to say just s, but that was something defined in the "Housekeeping.R" script and it does the weird 512 bins bias correction which makes no sense to me
...
for(i in 1:length(filled$Date)){
  ...
  s_original[i] <- entropyprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),timestep=i,stat="Shannon") #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
  bias[i] <- mean(out) - s_original[i] #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
  sorted <- sort(out)
  b1[i] <- sorted[25]
  b2[i] <- sorted[975]
  pb$tick()
...
#plot.ts(b2-bias+y_offset,ylim=c(min(b1-bias)+y_offset,max(b2-bias)+y_offset))
plot.ts(b2+y_offset,ylim=c(min(b1)+y_offset,max(b2)+y_offset))
#lines(b1-bias+y_offset)
lines(b1+y_offset)
lines(s_original+y_offset,col="red") #Why is the bias needed?
...
for(i in 1:length(filled$Date)){
  ...
  bias[i] <- mean(out) - entropyprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),timestep=i,stat="Negent") #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
  sorted <- sort(out)
  b1[i] <- sorted[25]
  b2[i] <- sorted[975]
  pb$tick()
...
for(i in 1:length(filled$Date)){
  cumulative_gaussianKL <- c(cumulative_gaussianKL,entropyprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),timestep=i,stat="Negent"))
}

bias = rep(0,length(filled$Date)) #can comment out if you like
...
for(i in 1:length(filled$Date)){
  ...
  shannon_out <- out[,1]
  bias_2[i] <- mean(shannon_out) - entropyprocedure_2(filled[,-1],seq(1,length(filled[,-1]),length.out=length(filled[,-1])),timestep=i,stat="Negent") #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
...
PDF_negent <- c()
for(i in 1:length(filled$Date)){
  PDF_negent <- c(PDF_negent,entropyprocedure_2(filled[,-1],seq(1,53,length.out=53),i,"Negent"))
}
...
plot(x=upper_sum_incidence_rate,y=upper_shannon+y_offset_2,ylim=c(min(lower_shannon)+y_offset_2,max(upper_shannon)+y_offset_2),pch=".") #type="l"?
points(x=lower_sum_incidence_rate,y=lower_shannon+y_offset_2,pch=".")
points(x=sum_cv,y=PDF_negent+y_offset_2,col="red",pch=".")

bias_2 <- rep(0,length(filled$Date)) #can comment out if you want bias
...
shannonBySestiere <- function(){
  ...
  for(s in unique(sestiere_df$Sestiere)){
    pb <- progress_bar$new(total=length(filled$Date))
    for(i in 1:length(filled$Date)){
      out_vec[i,match(s,unique(sestiere_df$Sestiere))] <- entropyprocedure_2(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))],seq(1,ncol(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))]),length.out=ncol(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))])),timestep=i,stat="Shannon")
      pb$tick()
    }
  }
  ed <- Sys.time()
...
plot.ts(x[[1]][,1],col="green",lwd=0.5,ylim=c(0,2),lty=2)
lines(x[[2]][,1],col="green",lwd=0.5,lty=2)
polygon(c(time_series,rev(time_series)),c(x[[1]][,1],rev(x[[2]][,1])),col=rgb(0,255,0,maxColorValue=255,alpha=100),lty=0)
lines(y[,1],col="green",lwd=2)
lines(x[[1]][,2],col="red",lwd=0.5,lty=2)
lines(x[[2]][,2],col="red",lwd=0.5,lty=2)
polygon(c(time_series,rev(time_series)),c(x[[1]][,2],rev(x[[2]][,2])),col=rgb(255,0,0,maxColorValue=255,alpha=100),lty=0)
lines(y[,2],col="red",lwd=2)
lines(x[[1]][,3],col="blue",lwd=0.5,lty=2)
lines(x[[2]][,3],col="blue",lwd=0.5,lty=2)
polygon(c(time_series,rev(time_series)),c(x[[1]][,3],rev(x[[2]][,3])),col=rgb(0,0,255,maxColorValue=255,alpha=100),lty=0)
lines(y[,3],col="blue",lwd=2)
lines(x[[1]][,4],col="yellow",lwd=0.5,lty=2)
lines(x[[2]][,4],col="yellow",lwd=0.5,lty=2)
polygon(c(time_series,rev(time_series)),c(x[[1]][,4],rev(x[[2]][,4])),col=rgb(255,255,0,maxColorValue=255,alpha=100),lty=0)
lines(y[,4],col="yellow",lwd=2)
lines(x[[1]][,5],col="orange",lwd=0.5,lty=2)
lines(x[[1]][,5],col="orange",lwd=0.5,lty=2)
polygon(c(time_series,rev(time_series)),c(x[[1]][,5],rev(x[[2]][,5])),col=rgb(255,165,0,maxColorValue=255,alpha=100),lty=0)
lines(y[,5],col="orange",lwd=2)
lines(x[[1]][,6],col="pink",lwd=0.5,lty=2)
lines(x[[2]][,6],col="pink",lwd=0.5,lty=2)
polygon(c(time_series,rev(time_series)),c(x[[1]][,6],rev(x[[2]][,6])),col=rgb(255,192,203,maxColorValue=255,alpha=100),lty=0)
lines(y[,6],col="pink",lwd=2)
legend(900,0.9,legend=unique(sestiere_df$Sestiere),col=c("green","red","blue","yellow","orange","pink"),lty=1,cex=0.5)
...
for(i in 1:length(filled$Date)){
  ...
  shannon_out <- out[,1]
  bias_2[i] <- mean(shannon_out) - entropyprocedure_2(filled[,-1],seq(1,length(filled[,-1]),length.out=length(filled[,-1])),timestep=i,stat="Shannon") #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
  shannon_order <- order(shannon_out)
  sorted_sum_incidence_rate <- out[,2][shannon_order]
  sorted <- shannon_out[shannon_order]
  lower_shannon[i] <- sorted[25]
...
y_offset_2 <- 0
#plot(x=upper_sum_incidence_rate,y=upper_shannon-bias_2+y_offset_2,ylim=c(min(lower_shannon)+y_offset_2,max(upper_shannon)+y_offset_2),pch=".") #type="l"?
plot(x=upper_sum_incidence_rate,y=upper_shannon+y_offset_2,ylim=c(min(lower_shannon)+y_offset_2,max(upper_shannon)+y_offset_2),pch=".")
#points(x=lower_sum_incidence_rate,y=lower_shannon-bias_2+y_offset_2,pch=".")
points(x=lower_sum_incidence_rate,y=lower_shannon+y_offset_2,pch=".")
points(x=sum_cv,y=s_original+y_offset_2,col="red",pch=".")
...
quantilefunctionprocedure_2 <- function(original_data,indices,timestep,stat=c("Shannon","Cases","Negent","UniformKL","Shannon_and_Cases","UniformKL_and_Cases")){
  boot_data <- original_data[,indices]
  colnames(boot_data) <- sub("\\.+[0-9]+$","",colnames(boot_data))
  pop_spec <- as.data.frame(matrix(nrow=ncol(boot_data),ncol=2))
  colnames(pop_spec) <- c("Parish","Population")
  ...
  if(stat == "Cases"){
    ...
    for(i in 1:length(cv)){
      if(i %% 2 == 0){
        cv_first_half <- c(cv_first_half,cv_ordered[i])
      } else {
        cv_second_half <- c(cv_second_half,cv_ordered[i])
      }
    }
    cv_second_half <- sort(cv_second_half,decreasing=TRUE)
    new_cv <- c(cv_first_half,cv_second_half)
    cv_norm <- new_cv/sum(new_cv)
    x <- seq(0,max(cv_norm),length.out=length(cv_norm))
    y <- dnorm(x,mean=mean(cv_norm),sd=sd(cv_norm))
    y <- y/sum(y)
    return(KL_new(cv_norm,y))
    #v <- -1*sum(cv_norm*log(y/(cv_norm/(max(cv_norm)/512))))
    #return(v)
  }
}
...
manual_boot_2 <- function(data,N,ts,st){
  vec <- rep(0,N)
  for(i in 1:N){
    vec[i] <- quantilefunctionprocedure_2(data,sample(ncol(data),replace=TRUE),ts,st)
  }
  return(vec)
}
...
manual_boot_two_outputs_2 <- function(data,N,ts,st){
  ...
  v2 <- rep(0,N)
  for(i in 1:N){
    x <- quantilefunctionprocedure_2(data,sample(ncol(data),replace=TRUE),ts,st)
    v1[i] <- x[1]
    v2[i] <- x[2]
  }
  return(cbind(v1,v2))
...
for(d in 1:length(filled$Date)){
  ...
  bias_quantile[d] <- mean(out) - quantilefunctionprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),d,"Shannon")
  pb$tick()
}
...
for(i in 1:length(filled$Date)){
  ...
  bias[i] <- mean(out) - quantilefunctionprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),timestep=i,stat="UniformKL") #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
  sorted <- sort(out)
  b1[i] <- sorted[25]
  b2[i] <- sorted[975]
  pb$tick()
...
for(i in 1:length(filled$Date)){
  quantile_uniformKL <- c(quantile_uniformKL,quantilefunctionprocedure_2(filled[,-1],seq(1,53,length.out=53),i,"UniformKL"))
}
...
for(i in 1:length(filled$Date)){
  ...
  shannon_out <- out[,1]
  bias_2[i] <- mean(shannon_out) - quantilefunctionprocedure_2(filled[,-1],seq(1,length(filled[,-1]),length.out=length(filled[,-1])),timestep=i,stat="UniformKL") #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
  shannon_order <- order(shannon_out)
  sorted_sum_incidence_rate <- out[,2][shannon_order]
  sorted <- shannon_out[shannon_order]
  lower_shannon[i] <- sorted[25]
...
for(d in 1:length(filled$Date)){
  ...
  lower_bound_quantile[d] <- sorted[25]
  upper_bound_quantile[d] <- sorted[975]
  bias_quantile[d] <- mean(out) - quantilefunctionprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),d,"Negent")
  cumulative_gaussianKL_quantile <- c(cumulative_gaussianKL_quantile,quantilefunctionprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),d,"Negent"))
  pb$tick()
}
...
for(d in 1:length(filled$Date)){
  ...
  bias_quantile[d] <- mean(out_2) - quantilefunctionprocedure_2(filled[,-1],seq(1,ncol(filled[,-1]),length.out=ncol(filled[,-1])),d,"Shannon")
  pb$tick()
}
...
quantile_by_sestiere_boot <- function(){
  ...
  for(s in unique(sestiere_df$Sestiere)){
    ...
    for(i in 1:length(filled$Date)){
      ...
      bias[i,match(s,unique(sestiere_df$Sestiere))] <- mean(out) - quantilefunctionprocedure_2(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))],seq(1,ncol(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))]),length.out=ncol(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))])),timestep=i,stat="Shannon") #check if entropy procedure does the same thing as what produces s exactly. Do b1 - bias and b2 - bias
      sorted <- sort(out)
      b1[i,match(s,unique(sestiere_df$Sestiere))] <- sorted[25]
      b2[i,match(s,unique(sestiere_df$Sestiere))] <- sorted[975]
      pb$tick()
...
quantile_by_sestiere <- function(){
  ...
  for(s in unique(sestiere_df$Sestiere)){
    pb <- progress_bar$new(total=length(filled$Date))
    for(i in 1:length(filled$Date)){
      out_vec[i,match(s,unique(sestiere_df$Sestiere))] <- quantilefunctionprocedure_2(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))],seq(1,ncol(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))]),length.out=ncol(filled[,match(sestiere_df$Parish[sestiere_df$Sestiere == s],colnames(filled))])),timestep=i,stat="Shannon")
      pb$tick()
    }
  }
  return(out_vec)
...
legend(800,1,legend=unique(sestiere_df$Sestiere),col=c("green","red","blue","yellow","orange","pink"),lty=1,cex=0.5)
...
# 
# start_time <- Sys.time()
# entropyprocedure_2(filled_transposed,seq(1,52,length.out=52),timestep=1,stat="Shannon")
# end_time <- Sys.time()
# time_taken <- round(end_time-start_time,2)
# time_taken
# 
# set.seed(123)
# results <- boot(data=filled_transposed,statistic=entropyprocedure_2,R=999,timestep=1,stat="Shannon")
# results
# summary(results)
# plot(results)
# 
